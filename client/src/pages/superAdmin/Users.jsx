        ...
        <div className="fixed inset-0 z-[100] bg-black/75 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white max-w-2xl w-full rounded-2xl shadow-2xl border-2 border-[#D4AF37] overflow-hidden max-h-[94vh] flex flex-col">
            ...
            <form onSubmit={handleCreate} className="p-5 overflow-y-auto flex-1 space-y-5">
              ...
              {/* 🎯 MANDATORY PORTAL SELECTION */}
              <div className="bg-gradient-to-r from-[#0F2038] via-[#1E3A5F] to-[#0F2038] text-white p-4 rounded-2xl border-2 border-[#D4AF37] space-y-3 shadow-md">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <FiShield className="text-[#D4AF37]" size={16} />
                    <span className="text-xs font-black uppercase tracking-wider text-[#D4AF37]">
                      Step 1: Select Target Admin Portal Access (1 of 3 Main Admin Portals)
                    </span>
                  </div>
                  <span className="text-[9px] font-bold bg-[#D4AF37] text-[#0F2038] px-2 py-0.5 rounded uppercase">
                    Mandatory Redirect
                  </span>
                </div>

                <p className="text-[11px] text-gray-300">
                  Whichever portal Super Admin selects here, when this officer logs in with their credentials, they will be redirected <strong>directly to that specific portal</strong>!
                </p>
                  ...
                  {[
                    {
                      ...
                      role: 'INSPECTION_OFFICER',
                      title: 'Inspector Dashboard Admin',
                      desc: 'DCP Inspection Officers for zone safety audits & public complaints.',
                      path: '/dashboard/inspector',
                      icon: '🛡️',
                      defaultDesignation: 'Deputy Commissioner of Police',
                    },
                    {
                      key: 'DISTRICT_ADMIN',
                      role: 'DISTRICT_ADMIN',
                      title: 'District Authority Admin',
                      desc: 'Commissioner of Police (CP), Joint CP & DM for district oversight.',
                      path: '/dashboard/district-admin',
                      icon: '🏛️',
                      defaultDesignation: 'District Authority Officer',
                    },
                    {
                      key: 'SUPER_ADMIN',
                      role: 'SUPER_ADMIN',
                      title: 'Super Admin Control Portal',
                      desc: 'State Command Level — Full control over user accounts & state system.',
                      path: '/dashboard/super-admin',
                      icon: '⚡',
                      defaultDesignation: 'State System Administrator',
                    },
                  ].map(portal => {
                    const isSelected = (form.assignedPortal || form.role) === portal.key;
                    return (
                      <div
                        key={portal.key}
                        onClick={() => {
                          setForm(f => ({
                            ...f,
                            assignedPortal: portal.key,
                            role: portal.role,
                            designation: f.designation || portal.defaultDesignation,
                          }));
                        }}
                        className={`p-3 rounded-xl border-2 cursor-pointer transition-all duration-200 text-left flex flex-col justify-between ${
                          isSelected
                            ? 'bg-white text-[#0F2038] border-[#D4AF37] shadow-lg ring-2 ring-[#D4AF37]'
                      ...
                      >
                        <div>
                          <div className="flex items-center justify-between mb-1">
                            ...
                            {isSelected && <FiCheck className="text-emerald-600 bg-emerald-100 p-0.5 rounded-full" size={14} />}
                          </div>
                          <p className={`text-xs font-black ${isSelected ? 'text-[#0F2038]' : 'text-white'}`}>
                            {portal.title}
                          </p>
                          <p className={`text-[9px] mt-1 ${isSelected ? 'text-gray-600' : 'text-gray-300'}`}>
                            {portal.desc}
                          </p>
                        </div>

                        <div className="mt-2 pt-2 border-t border-gray-200/30 flex items-center justify-between">
                          <span className={`text-[9px] font-mono font-bold ${isSelected ? 'text-blue-800' : 'text-[#D4AF37]'}`}>
                            {portal.path}
                          </span>
                          <span className={`text-[9px] font-black uppercase px-1.5 py-0.5 rounded ${
                            isSelected ? 'bg-[#0F2038] text-[#D4AF37]' : 'bg-white/20 text-white'
                          }`}>
